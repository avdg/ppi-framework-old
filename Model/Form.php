...
class PPI_Model_Form  {
	...
	function setFormStructure($structure = false) {
		if($structure === false) {
			$oForm 				= new PPI_Model_Form_Form($this->_formName);
		...
		if(array_key_exists('fields', $structure)) {
			...
			throw new PPI_Exception("Unable to find any fields when setting form structure for <strong>{$this->_formName}</strong>");
		}
	}
	...
	function setDefaults(array $defaults) {
		...
		if(!empty($this->_formStructure) && array_key_exists('fields', $this->_formStructure)) {
			...
			throw new PPI_Exception("Error setting form defaults");
		}
		$this->refreshDefaults();
	}
				...
				switch($ruleOptions['type']) {
					...
					case 'regex':
						if(!array_key_exists('pattern', $ruleOptions)) {
							throw new PPI_Exception('Error when trying to validate the form: no regex pattern found');
						}
						break;

					case 'compare':
						if(!array_key_exists('comparee', $ruleOptions)) {
							throw new PPI_Exception('Comparee not found for \'compare\' clause on '.$fieldName);
						}
						if(!array_key_exists($ruleOptions['comparee'], $this->_formFields)) {
							throw new PPI_Exception('Error when trying to compare '.$fieldName.'. Unable to find the comparee field, '.$ruleOptions['comparee']);
						}
					...
					default:
						throw new PPI_Exception('Error when validating the form, trying to use rule type: '. $ruleOptions['type'] . ' that doesn\'t exist');
						break;
				}
			}
		}
